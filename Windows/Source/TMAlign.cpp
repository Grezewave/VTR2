...
"\n"
" *********************************************************************\n"
" * TM-align (Version 20190724): protein structure alignment          *\n"
...
"          View superposed C-alpha traces of aligned regions by RasMol:\n"
"            $ rasmol -script TM_sup\n"
"          View superposed C-alpha traces of all regions by RasMol:\n"
"            $ rasmol -script TM_sup_all\n"
"          View superposed full-atom structures of aligned regions by RasMol:\n"
"            $ rasmol -script TM_sup_atm\n"
"          View superposed full-atom structures of all regions by RasMol:\n"
"            $ rasmol -script TM_sup_all_atm\n"
"          View superposed full-atom structures of all regions, including\n"
"          ligands, by RasMol:\n"
"            $ rasmol -script TM_sup_all_atm_lig\n"
"          View superposed atomic structures by PyMOL:\n"
"            $ pymol TM_sup_all_atm_lig.pml\n"
"          Generate image for superposed atomic structures by PyMOL:\n"
"            $ pymol -c TM_sup_all_atm_lig.pml -g TM_sup_all_atm_lig.png\n"
"\n"
    ...
    const double rmsd, const double TM1, const double Liden)
{
    ...
    buf_pymol<<"#!/usr/bin/env pymol\nload "<<fname_super
        <<".pdb\nload "<<yname<<"\nhide all\nshow cartoon\ncolor red\n"
        <<"color blue, "<<fname_super<<"\nset stick_radius, 0.25\n"
        <<"set sphere_scale, 0.25\nshow stick, not polymer\n"
        <<"show sphere, not polymer\nzoom\nset ray_shadow, 0"<<endl;
    ofstream fp;
        ...
        else if (line.compare(0,5,"loop_")==0) // PDBx/mmCIF
        {
            ...
            while(1)
            {
                    ...
                    line_vec[_atom_site["label_alt_id"]]=="A")
                {
                    ...
                    buf_all_atm_lig<<left<<setw(6)
                        <<line_vec[_atom_site["group_PDB"]]<<right
                        <<setw(5)<<(lig_idx1+lig_idx2)%100000<<' '<<atom<<' '
                        <<AA<<" B"<<resi<<inscode<<"   "
                        <<setw(8)<<line_vec[_atom_site["Cartn_x"]]
                        <<setw(8)<<line_vec[_atom_site["Cartn_y"]]
                        <<setw(8)<<line_vec[_atom_site["Cartn_z"]]<<'\n';
    ...
    fp<<buf_all_atm.str();
    fp.close();
    fp.open((fname_super+"_results.pdb").c_str());
